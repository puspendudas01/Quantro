        ...
        <div>
          {loading ? (
            ...
            questions.map((q, i) => (
              ...
              <div style={{ padding: '12px 16px', borderBottom: '1px solid var(--border-light)' }}>
                <div style={{ display: 'flex', alignItems: 'flex-start', gap: 10 }}>
                  ...
                  <div style={{ flex: 1 }}>
                    {q.hasQuestionImage && (
                      <img
                        src={`/questions/${q.id}/image`}
                        alt="Question"
                        draggable={false}
                        onMouseDown={e => e.preventDefault()}
                        onClick={e => openImageModal(`/questions/${q.id}/image`, e)}
                        ...
                        <div key={oi} style={{ fontSize: 11, padding: '4px 8px', borderRadius: 3, background: q.correctOptionIndex === oi ? '#dcfce7' : '#f1f5f9', color: q.correctOptionIndex === oi ? '#166534' : '#475569', fontWeight: q.correctOptionIndex === oi ? 700 : 400, display: 'flex', flexDirection: 'column', gap: 4, minWidth: 90 }}>
                          ...
                          {q.optionHasImage && q.optionHasImage[oi] && (
                            <img
                              ...
                              draggable={false}
                              onMouseDown={e => e.preventDefault()}
                              onClick={e => openImageModal(`/questions/${q.id}/option-image/${oi}`, e)}
                              onContextMenu={e => e.preventDefault()}
              ...
              {/* Image modal lightbox for teacher preview */}
              {imageModalSrc && (
                <div onClick={closeImageModal} style={{ position:'fixed', inset:0, background:'rgba(0,0,0,0.8)', display:'flex', alignItems:'center', justifyContent:'center', zIndex:9999 }}>
                  <img src={imageModalSrc} alt="Preview" style={{ maxWidth:'95%', maxHeight:'95%', borderRadius:8, boxShadow:'0 30px 80px rgba(0,0,0,0.6)' }} />
                </div>
              )}
              </React.Fragment>
            ))
