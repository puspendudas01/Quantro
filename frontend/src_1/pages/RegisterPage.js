        ...
        <div style={{ background:'#fff', borderRadius:12, boxShadow:'var(--shadow-md)', padding:32 }}>
          <form onSubmit={handleSubmit}>
            ...
            <div style={{ marginBottom:16 }}>
              ...
              <input type="email" value={form.email} onChange={e=>setForm(f=>({...f,email:e.target.value}))} required={form.role==='TEACHER'} placeholder={form.role==='STUDENT' ? 'xyz@domain.com' : 'you@example.com'} style={inp} />
            </div>
            ...
            {form.role === 'STUDENT' && (
              <>
                <div style={{ marginBottom:16 }}>
                  ...
                  <input value={form.enrollmentNo} onChange={e=>setForm(f=>({...f,enrollmentNo:e.target.value}))} required placeholder="12010002020210" style={inp} />
                </div>
                <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:12, marginBottom:16 }}>
                  <div>
                    <label style={lbl}>Stream</label>
                    <input value={form.stream} onChange={e=>setForm(f=>({...f,stream:e.target.value}))} required placeholder="CST" style={inp} />
                  </div>
                  <div>
                ...
                <div style={{ marginBottom:16 }}>
                  ...
                  <input value={form.studentYear} onChange={e=>setForm(f=>({...f,studentYear:e.target.value}))} required placeholder="Passout Year" style={inp} />
                </div>
                <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:12, marginBottom:16 }}>
                  <div>
