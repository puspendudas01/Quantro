...
export default function AdminDashboard() {
  ...
  const handleEditBlueprint = (bp) => {
    ...
    setBlueprintForm({
      ...
      optionShuffle: bp.optionShuffle !== false,
    ...
    <div style={{minHeight:'100vh',background:'var(--bg)',display:'flex',flexDirection:'column'}}>
      ...
      <div style={{flex:1,display:'flex',overflow:'hidden'}}>
        ...
        <div style={{flex:1,overflowY:'auto',padding:28}}>
          ...
          {tab === 'Blueprints' && (
            <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:24}}>
              <div style={{background:'#fff',padding:24,borderRadius:10,boxShadow:'var(--shadow-sm)',position:'relative'}}>
                {isEditingBlueprint && (
                  <button
                    type="button"
                    onClick={handleCancelBlueprintEdit}
                    aria-label="Back to create blueprint"
                    style={{position:'absolute',left:16,top:16,width:34,height:34,display:'flex',alignItems:'center',justifyContent:'center',border:'1px solid var(--border)',borderRadius:8,background:'#fff',color:'var(--text-secondary)',cursor:'pointer',fontSize:18,lineHeight:1}}
                  >
                    ←
                  </button>
                )}
                <h3 style={{fontSize:15,fontWeight:700,marginBottom:18, paddingLeft:isEditingBlueprint ? 44 : 0}}>{isEditingBlueprint ? 'Edit Blueprint' : 'Create Blueprint'}</h3>
                {isEditingBlueprint && (
                ...
                <form onSubmit={handleCreateBlueprint} style={{display:'flex',flexDirection:'column',gap:12}}>
                  ...
                  <div style={{display:'flex',alignItems:'center',gap:10,marginTop:2}}>
                    <input
                      id="blueprint-option-shuffle"
                      type="checkbox"
                      checked={blueprintForm.optionShuffle !== false}
                      onChange={e=>setBlueprintForm(f=>({...f,optionShuffle:e.target.checked}))}
                      style={{ width:16, height:16 }}
                    />
                    <label htmlFor="blueprint-option-shuffle" style={{...lbl, marginBottom:0, cursor:'pointer'}}>
                      Shuffle options for this blueprint
                    </label>
                  </div>
                  <div>
